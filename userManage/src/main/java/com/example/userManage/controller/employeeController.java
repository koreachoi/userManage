...
public class employeeController {
	...
	public String search(HttpSession session,
		...
		List<EmployeeDTO> employeesDTO = employeeService.getByFilter(employeeName, employeeName, company, jobType, zaisyoku);
		List<SettingDTO> syozokuKaisyaDTO = settingService.getSettingByCategoryList(1L, null, 1L);
		List<SettingDTO> syokugyoKindDTO = settingService.getSettingByCategoryList(3L,4L, null);
		UserDTO userDto = (UserDTO) session.getAttribute("loginUser");
		session.setAttribute("loginUser", userDto);
	...
	public String deleteEmployee(@RequestParam("syainId") Long syainId, RedirectAttributes redirectAttributes, HttpSession session, Model model) {
		UserDTO userDto = (UserDTO) session.getAttribute("loginUser");
		if (userDto == null) {
			System.out.println("null");
		}
